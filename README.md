...
STT (Speech-to-Text) inference plugin base framework. Provides the gRPC servicer, the `InferenceEngine` Protocol (interface), a Dummy engine for load testing, and an `entry_points`-based engine auto-discovery system.
...
│   ├── service/
│   │   └── inference_servicer.py # Transcribe + GetCapabilities + HealthCheck
│   └── engine/
│       ├── base.py               # InferenceEngine Protocol (interface)
│       ├── dummy.py              # DummySTTEngine (no model, fixed text)
│       └── registry.py           # entry_points-based engine auto-discovery
├── tests/
├── config/
│   ├── inference.yaml            # Real engine config (mlx-whisper default)
│   └── inference-dummy.yaml      # Dummy engine config (load testing)
└── pyproject.toml
```
...
```bash
# mlx-whisper engine (after installing plugin-stt-mlx-whisper)
...
Core sends an audio segment (extracted from the ring buffer after EPD triggers) and receives the full transcription.
...
- `engine_name` — e.g. `"mlx_whisper"`, `"faster_whisper"`, `"dummy_stt"`
...
```yaml
server:
  socket: /tmp/speechmux/stt-mlx.sock  # UDS socket path. Must match plugins.yaml endpoint socket.
  engine: mlx_whisper                   # Engine entry-point name (speechmux.stt_engine group).
  log_level: INFO                       # DEBUG | INFO | WARNING | ERROR
  max_concurrent_sessions: 10           # Max concurrent Transcribe requests.
  log_transcription_text: true          # Set to false to redact text from logs.

engine:
  mlx_whisper:
    model: mlx-community/whisper-large-v3-turbo  # HuggingFace model ID or local path.
    compute_type: float16               # float16 (Metal fp16) | float32 (fallback).
...
To implement a custom STT engine:
